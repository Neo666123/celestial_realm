...
local function ReturnPlayerToOverworld(inst, doer)
    ...
    StashInventoryToVault(doer, celestial_vault)
    RestoreInventoryFromVault(doer, overworld_vault)

    StashBuilderToVault(doer, celestial_vault)
    RestoreBuilderFromVault(doer, overworld_vault)

    -- 移除天界状态标签并还原老奶奶一本科技
    doer:RemoveTag("in_celestial_realm")
    ...
    local target_door = TheSim:FindFirstEntityWithTag("portal_entrance")
        or TheSim:FindFirstEntityWithTag("multiplayer_portal")
        or TheSim:FindFirstEntityWithTag("multiplayer_portal_moonrock")

    local dest_pos = target_door and target_door:GetPosition() or Vector3(0, 0, 0)
    dest_pos.x = dest_pos.x + 2.0
    ...
    doer.Transform:SetPosition(dest_pos.x, dest_pos.y, dest_pos.z)

    local arrive_fx = SpawnPrefab("spawn_fx_small")
    if arrive_fx then arrive_fx.Transform:SetPosition(dest_pos.x, dest_pos.y, dest_pos.z) end
    ...
    if doer.components.talker then
        doer.components.talker:Say("已返回大世界，大世界装备与科技已归还。")
    end
end
...
local function OnStartChanneling(inst, channeler)
    ...
    ReturnPlayerToOverworld(inst, channeler)
end
...
local function fn()
    ...
    inst.AnimState:SetMultColour(1.0, 0.75, 0.25, 1.0)
    inst.AnimState:SetBloomEffectHandle("shaders/anim.ksh")

    inst:AddTag("staysthroughvirtualrooms")
    inst:AddTag("portal_exit")
...
return Prefab("portal_exit", fn, assets)